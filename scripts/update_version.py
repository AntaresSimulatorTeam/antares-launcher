#!/usr/bin/python3
"""
Script used to update the version number and release date of Antares-Launcher.
"""
...
PROJECT_DIR = next(iter(p for p in HERE.parents if p.joinpath("antareslauncher").exists()))
...
def update_changelog(change_log: str, new_version: str, new_date: str) -> t.Generator[Token, None, None]:
    new_title = f"[{new_version}] - {new_date}"
...
def upgrade_version(new_version: str, new_date: str) -> None:
    ...
    fullpath = PROJECT_DIR.joinpath("antareslauncher/__init__.py")
    text = fullpath.read_text(encoding="utf-8")
    patched = re.sub(search, replace, text, count=1)
    fullpath.write_text(patched, encoding="utf-8")

    print("Preparing the CHANGELOG in the documentation...")
    changelog_path = PROJECT_DIR.joinpath("CHANGES.md")

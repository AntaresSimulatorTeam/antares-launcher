...
class TestIntegrationJobKilController:
    ...
    def test_job_kill_controller_kill_job_calls_connection_execute_command(
        ...
        # given
        job_id = 42
        self.job_kill_controller.env.connection.execute_command = mock.Mock(return_value=("", ""))
        # when
        self.job_kill_controller.kill_job(job_id)
        # then
        self.job_kill_controller.env.connection.execute_command.assert_called_once_with(f"scancel {job_id}")
